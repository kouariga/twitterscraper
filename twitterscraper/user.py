...
class User:
    ...
    def from_soup(self, tag_prof_header, tag_prof_nav):
        ...
        self.user = tag_prof_header.find(
            'a', {
                'class': 'ProfileHeaderCard-nameLink u-textInheritColor js-nav'})['href'].strip("/")
        self.full_name = tag_prof_header.find(
            'a', {'class': 'ProfileHeaderCard-nameLink u-textInheritColor js-nav'}).text
        ...
        date_joined = tag_prof_header.find(
            'div', {
                'class': "ProfileHeaderCard-joinDate"}).find(
            'span', {
        ...
        self.id = tag_prof_nav.find('div', {'class': 'ProfileNav'})[
            'data-user-id']
        tweets = tag_prof_nav.find('span',
        ...
        following = tag_prof_nav.find(
            'li', {
                'class': "ProfileNav-item ProfileNav-item--following"}). find(
            'span', {
                'class': "ProfileNav-value"})['data-count']
        ...
        followers = tag_prof_nav.find(
            'li', {
                'class': "ProfileNav-item ProfileNav-item--followers"}). find(
            'span', {
                'class': "ProfileNav-value"})['data-count']
        ...
        likes = tag_prof_nav.find(
            'li', {
                'class': "ProfileNav-item ProfileNav-item--favorites"}). find(
            'span', {
                'class': "ProfileNav-value"})['data-count']
        if likes is None:
            self.likes = 0
